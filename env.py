...
async def handle_request(request: Request):
    ...
    session_id = helper_fun.extract_session_id(output_contexts[0]["name"])


    
    intent_handler_dict = {
        'order.add- context: ongoing-order': add_to_order,
        'order.remove - contest: ongoing-order': remove_from_order,
...
#step 2: see if that id has order 
#step 3: 
...
def remove_from_order(parameters: dict, session_id: str):
    ...
    if len(current_order.keys()) == 0:
        ...
        order_str = helper_fun.get_str_from_food_dict(current_order)
...
def add_to_order(parameters, session_id):
    ...
    if len(food_items) != len(quantities):
        ...
        order_str = helper_fun.get_str_from_food_dict(inprogress_orders[session_id])
